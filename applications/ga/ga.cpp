...
typedef crsGA::GeneticAlgorithm<Gen, Chromosome, Population> GA;
...
struct UpdateChromosomeNode : public osg::NodeCallback
{
    GA *_ga;
    int _index;
    UpdateChromosomeNode(GA *ga) : _ga(ga), _index(-1) {}
    UpdateChromosomeNode(GA *ga, size_t index) : _ga(ga), _index(index) {}
    virtual void operator()(osg::Node *node, osg::NodeVisitor *)
    {
        ...
        if (geo)
        {
            if (_index == -1)
            {
                const auto &c = _ga->getPopulation().getFittestChromosome();
                updateGeometryWithChromosome(geo, c);
                return;
            }
            const auto &c = _ga->getPopulation().getChromosome(_index);
            updateGeometryWithChromosome(geo, c);
        }
    }
};
...
osg::ref_ptr<osg::Node> generatePopulationScene(GA *ga)
{
    osg::Group *g = new osg::Group();
    for (size_t i = 0; i < ga->getPopulation().getChromosomes().size(); i++)
    {
        auto cn = generateChromosomeGeometry(ga->getPopulation().getChromosome(0).getGenes().size(), osg::Vec4(0.0, 0.0, 0.0, 1.0), 1.0);
        updateGeometryWithChromosome(cn.get(), ga->getPopulation().getChromosome(i));
        cn->setUpdateCallback(new UpdateChromosomeNode(ga, i));
        g->addChild(cn.get());
    }
    return g;
}

osg::ref_ptr<osg::Node> generateFittestChromosomeScene(GA *ga, const osg::Vec4 &color = osg::Vec4(1.0, 0.0, 0.0, 1.0))
{
    auto cn = generateChromosomeGeometry(ga->getPopulation().getChromosome(0).getGenes().size(), color, 4.0);
    const auto &c = ga->getPopulation().getFittestChromosome();
    updateGeometryWithChromosome(cn.get(), c);
    cn->setUpdateCallback(new UpdateChromosomeNode(ga));
    return cn;
}
...
class UpdateOperation : public osg::Operation
{
  ...
  public:
    UpdateOperation(GA *ga, osgText::Text *fitnessText, double maxDuration)
    ...
    virtual void operator()(osg::Object *caller)
    {
        ...
        if (viewer)
        {
            ...
            _ga->step(time_span.count());
    }

    void update()
    {
        std::stringstream ss;
        ss << "Fitness = " << _ga->getPopulation().getFittestChromosome().getFitness();
...
int main(int argc, char **argv)
{
    ...
    GA ga(populationSize, numGenes, fitnessGoal);
    ga.setMutationFactor(genMutationProbability);
    ga.setUserData(pc);
    ...
    if (show)
    {
        ...
        g->addChild(generatePopulationScene(&ga));
        osg::Vec4f fittestColor = osg::Vec4(1.0, 0.0, 0.0, 1.0);
        g->addChild(generateFittestChromosomeScene(&ga, fittestColor));
        ...
        osg::ref_ptr<UpdateOperation> updateOperation = new UpdateOperation(&ga, fittestTextScene.get(), maxDuration);
    ...
    if (ga.isSolution(ga.getPopulation().getFittestChromosome()))
    {
        LOG_DEBUG("Solution found in generation ", ga.getGeneration());
    }
    else
    {
        LOG_DEBUG("Solution NOT found in generation ", ga.getGeneration());
    }
    LOG_DEBUG("Fitness=", ga.getPopulation().getFittestChromosome().getFitness(), " Chromosome: {", ga.getPopulation().getFittestChromosome(), "}");
